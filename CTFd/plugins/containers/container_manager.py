...
class ContainerManager:
    ...
    def run_command(func):
        """Decorator to ensure Docker connection is available before running commands"""
        def wrapper_run_command(self, *args, **kwargs):
            # Check if client exists and try to ping it
            ...
            try:
                ...
                try:
                    self.initialize_connection(self.settings, self.app)
                    # Test the new connection and retry the command
                    if self.client is not None:
                        self.client.ping()
                        return func(self, *args, **kwargs)
                except (ContainerException, Exception):
                    pass
                
                raise ContainerException(
                    f"Docker connection was lost: {str(e)}. Please try your request again later."
                )
