...
def load(app: Flask):
    ...
    def create_container(chal_id, team_id):
        ...
        # Insert placeholder record first to prevent TOCTOU race condition
        # This acts as a reservation to prevent concurrent container creation
        expires = int(time.time() + container_manager.expiration_seconds)
        placeholder_container = ContainerInfoModel(
            container_id="",  # Will be updated after container creation
            challenge_id=challenge.id,
            team_id=team_id,
            port=0,  # Will be updated after container creation
            timestamp=int(time.time()),
            expires=expires,
        )
        db.session.add(placeholder_container)
        
        try:
            db.session.commit()
        except Exception as err:
            # If commit fails, another request may have created a container concurrently
            db.session.rollback()
            # Check if a container was created by another request
            existing_container = ContainerInfoModel.query.filter_by(
                challenge_id=challenge.id, team_id=team_id
            ).first()
                ...
                return json.dumps(
                    {
                        ...
                        "expires": existing_container.expires,
                    }
                )
            return {"error": "Failed to reserve container slot"}, 500
        ...
        try:
            ...
            # Container creation failed, remove the placeholder record
            db.session.delete(placeholder_container)
            db.session.commit()
        ...
        # Update the placeholder record with actual container information
        placeholder_container.container_id = created_container.id
        placeholder_container.port = port
        db.session.commit()
    ...
    def route_update_settings():
        ...
        if container_expiration is None:
            ...
            container_expiration = ContainerSettingsModel(
                key="container_expiration",
                value=request.form.get("container_expiration"),
            )
